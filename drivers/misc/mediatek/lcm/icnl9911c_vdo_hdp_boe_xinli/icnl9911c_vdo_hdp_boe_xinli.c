...
static void lcm_init(void)
{
	...
	if (ret < 0)
		pr_debug("[LCM]icnl9911c--tps6132--cmd=%0x--i2c write error--\n",
				cmd);
	else
		pr_debug("[LCM]icnl9911c--tps6132--cmd=%0x--i2c write success--\n",
				cmd);
...
static unsigned int lcm_compare_id(void)
{
	...
	pr_info("[LCM]%s,icnl9911c id0 = 0x%x,id1 = 0x%x, id2 = 0x%x\n",
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_info("[LCM]%s,icnl9911c backlight: level = %d lcd_bl_en = %d\n", __func__, level, lcd_bl_en);
	if((0 != level) && (level <= 14))
		level = 14;
	level = level*72/100;
