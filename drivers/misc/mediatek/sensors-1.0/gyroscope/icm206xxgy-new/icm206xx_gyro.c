...
/* if use this type of enable, G sensor should report inputEvent(x, y, z, stats, div) to HAL */
...
#ifdef USE_OLD_SENSOR_DTS_ARCH
        int ret = 0;
	const char *name = "mediatek,icm206xxgy";

	ret = get_gyro_dts_func(name, hw);
	if (ret!=0)
		GYRO_PR_ERR("get icm206xx gyro dts info fail\n");
#endif
