...
#include <hwmsensor.h> //added by xen 20171009
...
#define ANDROID_O1_SUPPORT  //added by xen for android O1 platform driver 20180122
...
/*----------------------------------------------------------------------------*/
#define stk3x1x_DEV_NAME     "stk3338"
/*----------------------------------------------------------------------------*/
...
#define W2000_SUPPORT
#endif
#ifdef STK_TUNE0
#if defined(YK736_CUSTOMER_XINJIDE_G1_HDPLUS) //zwl 20180425
	#define STK_MAX_MIN_DIFF	200
	#define STK_LT_N_CT	80
	#define STK_HT_N_CT	130
#elif defined(YK685_CUSTOMER_CAIFU2_BSM01_HDPLUS)
	#define STK_MAX_MIN_DIFF	300 //165 //800//16
	#define STK_LT_N_CT	        120 //45//1300//8
	#define STK_HT_N_CT	        250 //115 //1500//12	
#else
	#define STK_MAX_MIN_DIFF	200
...
#define STK_IRC_ALS_CORREC		106//fae 106//2000
...
/*----------------------------------------------------------------------------*/
static int stk3x1x_i2c_suspend(struct device *dev);//struct i2c_client *client, pm_message_t msg);
static int stk3x1x_i2c_resume(struct device *dev);//struct i2c_client *client);
...
static const struct of_device_id alsps_of_match[] = {
	//{.compatible = "mediatek,alsps_stk3x1x"}, //modified by xen for test 20180125
...
#ifndef ANDROID_O1_SUPPORT  //Android O1 kernel code deleted by xen 20180122
...
int stk3x1x_read_als(struct i2c_client *client, u16 *data)
{
	...
	if(obj->ir_code)
	{
		#if 0//defined(Q812_CUSTOMER_CAIFU_KS680M_HD720)
		obj->als_correct_factor = 230;  //1000    //115  add for lux  by yx 20170712
		#elif defined(YK685_CUSTOMER_CAIFU2_BSM01_HDPLUS)
		obj->als_correct_factor = 5666;//606;
		#else
		obj->als_correct_factor = 330; //2000;
		#endif
...
#ifndef ANDROID_O1_SUPPORT  //xen 20180123
/*----------------------------------------------------------------------------*/
...
#ifndef CALI_EVERY_TIME
	atomic_set(&obj->ps_high_thd_val, obj->ps_stat_data[1] + 3*obj->stk_ht_n_ct); 
	atomic_set(&obj->ps_low_thd_val, obj->ps_stat_data[1] + 3*obj->stk_lt_n_ct); 		
#else				
    #if defined(YK685_CUSTOMER_CAIFU2_BSM01_HDPLUS)
	obj->ps_high_thd_boot = obj->ps_stat_data[1] + 3*obj->stk_ht_n_ct;
	obj->ps_low_thd_boot = obj->ps_stat_data[1] + 2*obj->stk_ht_n_ct + obj->stk_lt_n_ct;
	#else		
	obj->ps_high_thd_boot = obj->ps_stat_data[1] +3* obj->stk_ht_n_ct;
...
#if defined(CONFIG_OF)
	//enable_irq(obj->irq); //modified by xen 20170307
#endif
#endif
...
/* gpio setting */
	pinctrl = devm_pinctrl_get(&client->dev);//&alspsPltFmDev->dev);
	if (IS_ERR(pinctrl)) {
		ret = PTR_ERR(pinctrl);
		APS_ERR("Cannot find alsps pinctrl!\n");
	}
	printk("stk3x1x_setup_eint-1\n");
...
/* eint request */
	printk("stk3x1x_setup_eint-3\n");
	if (stk3x1x_obj->irq_node) {
		//node = of_find_matching_node(node, touch_of_match);
		printk("stk3x1x_obj->irq_node not eual ZERO!\n");
		...
		//enable_irq(stk3x1x_obj->irq); //modified by xen 20161209
	} else {
...
*/	
...
#ifdef W2000_SUPPORT
	data = 0x3F;
    err = stk3x1x_master_send(client, 0x4F, &data, 1);
	if (err < 0)
	{
		APS_ERR("write 0x4F = %d\n", err);
		return err;
	} 
	
	data = 0x01 ;
    err = stk3x1x_master_send(client, 0xFA, &data, 1);
	if (err < 0)
	{
    ...
    data = 0x10;
    err = stk3x1x_master_send(client, 0xA0, &data, 1);
	if (err < 0)
	{
		APS_ERR("write 0xA0 = %d\n", err);
		return err;
	} 
	
	data = 0x64;
    err = stk3x1x_master_send(client, 0xAA, &data, 1);
	if (err < 0)
	{
		APS_ERR("write 0xAA = %d\n", err);
		return err;
	} 
	
	data = 0x0F;
    err = stk3x1x_master_send(client, 0xA1, &data, 1);
	if (err < 0)
	{
		APS_ERR("write 0xA1 = %d\n", err);
		return err;
	} 
	
	data = 0x15;
    err = stk3x1x_master_send(client, 0xDB, &data, 1);
	if (err < 0)
	{
		APS_ERR("write 0xDB = %d\n", err);
		return err;
	} 
	
	
	data = 0xC0;
    err = stk3x1x_master_send(client, 0xF4, &data, 1);
	if (err < 0)
	{
		APS_ERR("write 0xF4 = %d\n", err);
		return err;
	} 
#endif
...
#endif
{
	...
	switch (cmd)
	{
		...
		case ALSPS_GET_PS_TEST_RESULT: 
			...
			//if(obj->ps > atomic_read(&obj->ps_high_thd_val)) //xjl 20170606 for factory mode alps
			if((dat1 == 0)||(dat == 0)||(dat == dat1))
				ps_result = 0;
			else	
				ps_result = 1;
...
/*----------------------------------------------------------------------------*/
#endif //ended by xen for Android O1
...
#ifdef ANDROID_O1_SUPPORT  //xen added 20180122
...
static int als_get_data(int* value, int* status)
{
	...
	{
		#if 1//defined(YK736_CUSTOMER_CAIFU_KS928A_HD640)
		if(obj->als < 5)		
		#else
		if(obj->als < 3)
		#endif
		{
			obj->als_last = obj->als;
			*value = 0;//stk3x1x_get_als_value(obj, 0);
		}
		else if(abs(obj->als - obj->als_last) >= 20)//STK_ALS_CODE_CHANGE_THD)//10
		{
			obj->als_last = obj->als;
		#if 1 //defined(Q812_CUSTOMER_CAIFU_KS680M_HD720)
			if(obj->als >10240)
			{
				*value = 10240;
			}
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	//obj->irq_node = of_find_compatible_node(NULL, NULL, "mediatek, als-eint"); //modified by xen for MT6739 20171026
	obj->irq_node = client->dev.of_node; //of_find_compatible_node(NULL, NULL, "mediatek,alsps");

	atomic_set(&obj->state_val, 0);
#if defined(YK736_CUSTOMER_XINJIDE_G1_HDPLUS) //zwl 20180425
        atomic_set(&obj->psctrl_val, 0x32);
#elif defined(W2000_SUPPORT)
        atomic_set(&obj->psctrl_val, 0xB2);// 0x31
#else
	atomic_set(&obj->psctrl_val, 0x31);
#endif
#ifdef W2000_SUPPORT
#if defined(YK685_CUSTOMER_CAIFU2_BSM01_HDPLUS)
	atomic_set(&obj->alsctrl_val, 0x30);
#else
	atomic_set(&obj->alsctrl_val, 0x32);
#endif
	obj->ledctrl_val = 0x60;	
	obj->wait_val = 0x1F; //0x7;
#else
	atomic_set(&obj->alsctrl_val, 0x39);
	obj->ledctrl_val = 0xFF;	
	obj->wait_val = 0xF; //0x7;
#endif
	obj->int_val = 0;
	obj->first_boot = true;	
	...
	obj->als_correct_factor = 5666;//606;	
	#else
	obj->als_correct_factor = 190; //205; //330; //2000; //1000
	#endif
...
#if defined(ANDROID_O1_SUPPORT) //modified by xen 20180122
...
//	struct alsps_hw *hw = get_cust_alsps_hw();
	...
	printk("stk3x1x_local_init stk3x1x_init_flag=%d\n",stk3x1x_init_flag); 
...
static int __init stk3x1x_init(void)
{
	...
	printk("stk3x1x_init start\n");
	ret = get_alsps_dts_func(name, hw);
	if (ret)
